...
#Get factors in descending order (avoid ggplot default to alphabetical)
data$Breed <- factor(data$Breed, levels=data$Breed)

#Get names of columns, replace for radar plot use
names(data)[1]<-"group"
...
Mainplot <- data %>%
  ...
  theme(legend.position="none",
        ...
        plot.subtitle= element_text(family = "Cabin", hjust=0.5, margin=margin(0,0,30,0)),
        plot.caption=element_text(family = "Cabin", size=10),
        strip.text = element_text(size = 10,  family = "Cabin", face = "bold"),
        plot.margin=margin(20,0,10,0),
        plot.title=element_text(family = "Cabin", hjust=0.5, face="bold", size=18, margin=margin(0,0,10,0)))

#Annotated plot
ggdraw(Mainplot) +
  draw_image("/Users/abisesi/Desktop/PhD/Non-Dissertation Work/Programming Repos/R/Tidy-Tuesday-2022/Week 5/AKC.png",
