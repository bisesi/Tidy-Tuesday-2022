#ATB
#Tidy Tuesday W9 2022
#EV charging stations

#load packages
library("tidyverse")
library("sf")
library("maps")
library("ggspatial")
library("rnaturalearth")
library("showtext")
library("ggtext")

#load data
stations <- readr::read_csv('https://raw.githubusercontent.com/rfordatascience/tidytuesday/master/data/2022/2022-03-01/stations.csv')

citycounts <- stations %>%
  select(c(LONGITUDE, LATITUDE, CITY, STATE)) %>%
  group_by(CITY) %>%
  count(CITY)

locations <- citycounts %>%
  full_join(stations) %>%
  select(CITY, n, LONGITUDE, LATITUDE, STATE) %>%
  distinct(CITY, .keep_all = TRUE)
  
#get state data
world_map_data <- ne_countries(scale = "medium", returnclass = "sf")
state_map_data <- map('state', fill = TRUE, plot = FALSE) %>% st_as_sf()

positions <- locations  %>%
  filter(!STATE %in% c("DC","PR", "HI", "AK", "AS", "VI", "GU", "MP"))
  
#Formmating
font_add_google("Cabin")
showtext_auto()

#Plot
plot <- ggplot() +
  geom_sf(data = state_map_data, fill = "white") +
  geom_point(data = positions, aes(x = LONGITUDE, y = LATITUDE), size = 1, color = "#00a6c0", alpha = 0.15)+
  geom_point(data = positions, aes(x = LONGITUDE, y = LATITUDE), size = 1, shape = 1, color = "#00a6c0")+
  coord_sf(xlim = c(-125, -66), ylim = c(24, 50))+
  labs(title = "EV Charging Stations",
       subtitle = "Density of alternative transportation fuel stations in the continental US",
       caption = "Data: US DOT | Visualization: A Bisesi")+
  theme(text = element_text(family = 'Cabin'),
        panel.grid.major = element_line(color = 'white',
                                        linetype = 'dashed',
                                        size = .3),
        panel.background = element_rect(fill = 'white'),
        plot.title = element_text(size = 32),
        plot.subtitle = element_text(size = 16),
        axis.title = element_blank(),
        axis.text = element_blank(), 
        axis.ticks = element_blank())

